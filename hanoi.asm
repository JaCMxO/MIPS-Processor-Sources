...
main:
	...
	add		$a0, $zero, $s0			#pass disc number as argument
	addi	$a1, $s1, 4				#get tower B direction
	jal		zeroHanoi				#call zeroHanoi routine
	add		$a0, $zero, $s0			#pass disc number as argument
	addi	$a1, $s1, 8				#get tower B direction
	jal		zeroHanoi				#call zeroHanoi routine	
...
zeroCycle:
	...
	bne		$t1, $zero, return		#t1 != 0 ? jump:continue
	addi	$t0, $t0, 1				#increment control variable
	sw		$zero, ($a1)			#store zero in tower
	addi	$a1, $a1, 0x20			#point to next store location
	j		zeroCycle				#repeat cycle
